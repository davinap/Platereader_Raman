...
ggplotify.bc<-function(p){ggplot(p, aes(Time,value, col=variable)) + 
    ...
    theme_bw() +
    mytheme +
    scale_x_continuous(limits = c(0, 42), breaks = seq(0, 42, by = 8)) +
...
sortfiles<-mixedsort(list.files(path ="C:/Users/davin/Documents/PhD/Results/byonoy/151022/", pattern = '151022_2_t'))
sortfiles
...
tidy.up<-function(newseries){
  ...
  write.csv(num, "tidy_new2.csv", row.names=FALSE) #######CHANGE FILE NAME
}
...
tidy.up(series)
tidied<-read.csv(file = "tidy_new2.csv")
